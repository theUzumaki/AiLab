...
sys.path.append(ABSOLUTE_PATH + "/Object_detection/CNN")
...
class ObjectDetector:
    ...
    def postprocess_predictions(self, predictions, original_size, conf_threshold=0.5):
        ...
        # Sposta tutto sul device corretto
    ...
    def predict_single_image(self, image_path, conf_threshold=0.5):
        ...
        with torch.no_grad():
            ...
            boxes = det_boxes[0].to(self.device)
            labels = det_labels[0].to(self.device)
            scores = det_scores[0].to(self.device)
        # Post-processing
