...
from myCallback import MyCallbacks  # Custom callbacks for RLlib
...
def env_creator(agent):
    ...
    config = (
        ...
        .env_runners(
            ...
            rollout_fragment_length=1,  # lunghezza del frammento di rollout
        ...
        .training(
            train_batch_size=2,
            num_sgd_iter=1,
            minibatch_size=2,  # dimensione del minibatch per l'aggiornamento SGD
        )
    )
...
def trainable(agent):
    ...
    register_env("KillerVictimEnv", KillerVictimEnv)

    print(f"[INFO] Starting training for {agent} agent...")
