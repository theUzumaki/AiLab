...
from new_env import KillerVictimEnv
...
def trainable(agent: str):
    ...
    # model = PPO.load("./model_game/ppo_" + agent, env=env, verbose=2, learning_rate=0.0003, n_steps=2048 if agent == "victim" else 1024, batch_size=128, ent_coef=0.02, clip_range=0.2, gae_lambda=0.98)

    for _ in range(20):
        model = PPO.load(
            "./model_game/ppo_" + agent + "_1",
            env=env,
            verbose=2,
            learning_rate=0.0003,
            n_steps=2048 if agent == "victim" else 512,
            batch_size=128,
            ent_coef=0.02,
            clip_range=0.2,
            gae_lambda=0.98,
        ...
        # Training
        model.learn(
            callback=StopEp(n_episodes=10, agent=agent),
            progress_bar=True,
        ...
        model.save("./model_game/ppo_" + agent + "_1")
